test_that("construtor has sensible defaults", {
  ...
  step <- h2oplyr:::step_subset(first)
...
test_that("generates expected calls", {
  first <- lazy_dt(data.frame(x = 1), "DT")

  ungrouped <- h2oplyr:::step_subset(first, i = quote(i), j = quote(j))
  expect_equal(h2oplyr:::dt_call(ungrouped), expr(DT[i, j]))

  with_i <- h2oplyr:::step_subset(first, i = quote(i), j = quote(j), groups = "x")
  expect_equal(h2oplyr:::dt_call(with_i), expr(DT[, .SD[i, j], keyby = .(x)]))

  without_i <- h2oplyr:::step_subset(first, j = quote(j), groups = "x")
  expect_equal(h2oplyr:::dt_call(without_i), expr(DT[, j, keyby = .(x)]))
})
...
test_that("arrange doesn't use, but still preserves, grouping", {
  dt <- group_by(lazy_dt(data.frame(x = 1, y = 2), "DT"), x)

  step <- arrange(dt, y)
  expect_equal(step$groups, "x")
  expect_equal(h2oplyr:::dt_call(step), expr(DT[order(y)]))

  step2 <- arrange(dt, y, .by_group = TRUE)
  expect_equal(h2oplyr:::dt_call(step2), expr(DT[order(x, y)]))
})
...
test_that("empty summarise returns unique groups", {
  dt <- lazy_dt(data.frame(x = c(1, 1, 2), y = 1, z = 1), "DT")
...
test_that("can slice when grouped", {
  dt <- lazy_dt(data.frame(x = 1:4, y = c(1, 2, 1, 2)), "DT")
...
test_that("basic usage generates expected calls", {
  dt <- lazy_dt(data.frame(x = 1:5, y = 1), "DT")
